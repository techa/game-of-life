...
function a11ySetActiveCell(node: HTMLElement, x: number, y: number): void {
	...
	// Target Element
	const targetRowElement = node.querySelector(
		`[aria-rowindex="${focusedElemRowIndex + y}"]`,
	)
	if (targetRowElement !== null) {
		const targetColElement: HTMLElement | null =
			targetRowElement.querySelector(
...
function a11yJumpToOuterColumn(
	node: HTMLElement,
